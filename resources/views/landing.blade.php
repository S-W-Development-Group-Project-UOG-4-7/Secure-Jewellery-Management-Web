...
<body>
    ...
    <nav class="navbar" id="navbar">
        ...
        <div class="nav-right">
            <button class="btn-nav" onclick="window.location.href='{{ route('login') }}'">Login</button>
    ...
    <section class="hero">
        ...
        <div class="hero-btns" style="opacity: 0; animation: fadeUp 1s 2.8s forwards; display: flex; gap: 20px;">
             <button class="btn-nav primary" style="padding: 15px 40px; font-size: 1rem;" onclick="window.location.href='{{ route('login') }}'">Enter Portal</button>
    ...
    <script>
        ...
        // 5. AUTH HANDLERS
        // Note: Login logic is effectively bypassed now since the button redirects.
        // We leave it here in case you revert to the modal later.
        document.getElementById('loginForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (res.ok && result.status === 'success') {
                    ...
                    msg.innerHTML = '<i class="fa-solid fa-check"></i> ACCESS GRANTED';
                    setTimeout(() => {
                        window.location.href = result.redirect || "{{ route('design.studio') }}";
                    }, 1000);

                } else {
                    msg.style.color = '#ff5252';
                    msg.innerText = result.message || 'Access Denied: Invalid Credentials';
                ...
                console.error(err);
                msg.style.color = '#ff5252';
                msg.innerText = "System Connection Error (500)";
                btn.innerHTML = 'INITIALIZE SESSION';
                btn.style.opacity = '1';
            }
        });

        // --- REGISTER LOGIC (Still Active for the Register Modal) ---
