...
import { 
  ...
  FaRing,
  FaTimes,
  FaDollarSign // <--- ADDED THIS FIXED IMPORT
} from "react-icons/fa";
...
export default function CustomOrderForm() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      console.error(e2);
      setErr(e2?.response?.data?.message || "Failed to submit order.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#e0e0e0] px-4 py-8 md:py-12 font-sans">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-[#111] border border-[#222] rounded-2xl p-6 md:p-10 shadow-2xl relative overflow-hidden">
          ...
          {msg && (
            <div className="mb-6 rounded-xl border border-green-500/30 bg-green-900/10 px-4 py-4 text-green-300 flex items-center gap-3 shadow-lg">
              <FaGem /> {msg}
            </div>
          )}
          {err && (
          ...
          <form onSubmit={submit} className="space-y-8 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Budget - NOW WORKS CORRECTLY */}
              <div>
                <label className="block text-xs uppercase tracking-wider text-gray-500 mb-2 flex items-center gap-2">
                  <FaDollarSign /> Est. Budget ($)
                </label>
                <input
                  type="number"
                  name="budget"
